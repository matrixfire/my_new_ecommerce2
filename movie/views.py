...
def blogs(request):
    ...
    if searchTerm:
        movies = Blog.objects.filter(title__icontains=searchTerm)
    else:
        movies = Blog.objects.all()
    return render(request, 'blogs/blogs.html',	{'searchTerm':searchTerm, 'movies': movies})
...
def detail(request, movie_id):
    movie = get_object_or_404(Blog,pk=movie_id)
